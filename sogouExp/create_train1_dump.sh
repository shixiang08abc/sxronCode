...
echo "`date "+%Y-%m-%d %H:%M:%S"`  start create train 1 dump rankmask=$rankmask"
filedir=$trainDir/RelevanceLTRData/codename/$datadir
if [ ! -d "$filedir" ];then
mkdir $filedir
fi

dumptrain=$filedir/qs.dump.train.1
echo "`date "+%Y-%m-%d %H:%M:%S"` send train 1 query reld on new query: $queryterm" >> $filedir/log
date >> $filedir/log
